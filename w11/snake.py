...
class Snake:
    def __init__(self):
        ...
        self.dy = 0
        self.dy2 = 0
        self.elements = [[100, 180]]
        self.elements2 = [[100, 720]]
        self.score = 0
        self.score2 = 0
        self.added = False
        self.added2 = False
    def draw_snake(self):
        for element in self.elements:
            pygame.draw.rect(screen, (0, 0, 255), (element[0], element[1], self.width, self.height))
    def draw_snake2(self):
        for element in self.elements2:
            pygame.draw.rect(screen, (255, 0, 0), (element[0], element[1], self.width, self.height))
...
def game_over():
    ...
    screen.blit(font.render("Record:" + str(snake.score) + ' || ' + str(snake2.score2), True, (255, 255, 255) ), (320, 670))
    with open("records.txt", 'r') as ff:
        txt = ff.read().split()
    screen.blit(font.render("The highest score:" + str(max(txt)), True, (255, 255, 255) ), (270, 720))
    pygame.display.flip()
    time.sleep(5)
    pygame.quit()
...
while running:
    ...
    if play2:
        ...
        if nowallsg:
            ...
            display_score(120, 50, snake2.score2)
        if easy:
            FPS = 5
            screen.blit(nowalls2, (0, 0))
            if wall_colision_1() or wall_colision_1_2():
                game_over()
            easy_level()
            snake_collison()
            snake_collison2()
        ...
        if medium:
            FPS = 7
            screen.blit(nowalls2, (0, 0))
            if (wall_colision_1() or wall_collision_2()) or (wall_colision_1_2() or wall_collision_2_2()):
                game_over()
        ...
        if hard:
            FPS = 11
            screen.blit(nowalls2, (0, 0))
            if (wall_colision_1() or wall_collision_3()) or (wall_colision_1_2() or wall_collision_3_2()):
                game_over()
            easy_level()
            hard_level()
            if food_wall_collision_3():
